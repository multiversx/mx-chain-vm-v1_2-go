...
func v1_2_transferValueExecute(
	...
	if functionLength > 0 {
		...
		if arwen.WithFault(err, context, runtime.SyncExecAPIErrorShouldFailExecution()) {
			return 1
		}
	}
...
func v1_2_checkNoPayment(context unsafe.Pointer) {
	...
	if vmInput.CallValue.Sign() > 0 {
		runtime := arwen.GetRuntimeContext(context)
		arwen.WithFault(arwen.ErrNonPayableFunctionEgld, context, runtime.BaseOpsErrorShouldFailExecution())
		return
	}
	if len(vmInput.ESDTTransfers) > 0 {
		runtime := arwen.GetRuntimeContext(context)
		arwen.WithFault(arwen.ErrNonPayableFunctionEsdt, context, runtime.BaseOpsErrorShouldFailExecution())
		return
	}
}
...
func v1_2_getBlockRandomSeed(context unsafe.Pointer, pointer int32) {
	...
	arwen.WithFault(err, context, runtime.BaseOpsErrorShouldFailExecution())
}
...
func doESDTTransferAndExecuteSynchronously(
	...
	if value.Cmp(arwen.Zero) > 0 {
		if arwen.WithFault(arwen.ErrTransferValueOnESDTCall, context, runtime.SyncExecAPIErrorShouldFailExecution()) {
			return 1
		}
	}
	if len(args) < 1 {
		if arwen.WithFault(arwen.ErrArgOutOfRange, context, runtime.SyncExecAPIErrorShouldFailExecution()) {
			return 1
		}
	}
	...
	case core.BuiltInFunctionESDTNFTTransfer:
		...
		if !bytes.Equal(destination, args[3]) {
			if arwen.WithFault(arwen.ErrFailedTransfer, context, runtime.SyncExecAPIErrorShouldFailExecution()) {
				return 1
			}
		}
	...
	default:
		if arwen.WithFault(arwen.ErrFuncNotFound, context, runtime.SyncExecAPIErrorShouldFailExecution()) {
			return 1
		}
	}
