...
func v1_2_sha256(context unsafe.Pointer, dataOffset int32, length int32, resultOffset int32) int32 {
	...
	crypto := wasmvm.GetCryptoContext(context)
	...
	if wasmvm.WithFault(err, context, runtime.CryptoAPIErrorShouldFailExecution()) {
		return 1
	}
...
func v1_2_verifySecp256k1(
	...
	if keyLength != secp256k1CompressedPublicKeyLength && keyLength != secp256k1UncompressedPublicKeyLength {
		wasmvm.WithFault(wasmvm.ErrInvalidPublicKeySize, context, runtime.ElrondAPIErrorShouldFailExecution())
		return 1
	}
